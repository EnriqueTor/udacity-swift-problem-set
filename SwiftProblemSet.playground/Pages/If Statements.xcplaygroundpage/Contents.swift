...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike == true && run && true {
        print("\(name) is ready to rock and roll! and also finish the duathlon.")
    }
    else {
        print("\(name) is not ready to rock and roll yet! and specially not ready to finish the duathlon.")
    }
